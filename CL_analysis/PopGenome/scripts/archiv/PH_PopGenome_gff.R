...
#READING IN GENOME
genome <- readVCF(vcf, numcols=10000, tid=contig, frompos=1, topos=ct_length, approx = TRUE, gffpath=gff)
genom_all <- genome 
...
genom_all <- diversity.stats(genom_all, keep.site.info = F, pi = T, subsites = 'intergenic')
genom_all <- neutrality.stats(genom_all, FAST = TRUE, subsites = 'intergenic')
