...
hap_the_dip <- function(samp, gt){
  ...
  shuffles <- sample(c(1,2), nrow(pseudo_dip), replace = T)
  reindx <- data.frame(indx1 = shuffles, indx2 = 3 - shuffles) %>% as.matrix()
  shuffled_pseudo_dip <- apply(pseudo_dip, 1, function(x)(return(sample(x)))) %>% t()
 
#  shuffled_pseudo_dip <- pseudo_dip
  #for (i in 1:nrow(pseudo_dip)){

#    shuffled_pseudo_dip[i,] <- pseudo_dip[i, reindx[i,]]
#  }

  colnames(shuffled_pseudo_dip) <- paste0(samp, "_hap", 1:2)
  return(shuffled_pseudo_dip)
}
