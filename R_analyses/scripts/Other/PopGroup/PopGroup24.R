...
#sce$new_clusters[sce$celltype %in% c("Mature_spermatids", "Early_spermatids", 
#sce$new_clusters[sce$celltype == "GSC_or_Early_spermatogonia"] <- "Germ"
#                                             "Early_spermatocytes")] <- "Germ"
...
celltypes <- c("Cyst", "Epithelial", "GSC_and_Spermatogonia", "Spermatocytes", "Spermatids")

pdf("plots/DEG/PCA_compiled.pdf", width = 14, height = 10)
ggarrange(plotlist =  PCAs[celltypes], common.legend = T)
dev.off()
...
seurat_marker$broad_type <- "Somatic"
seurat_marker$broad_type[seurat_marker$celltype %in% c(
  "Spermatids", "Spermatocytes", "GSC_and_Spermatogonia")] <- "Germ"
#sce$broad_type <- sce$celltype
seurat_marker_ss <- subset(seurat_marker, celltype %in% c("Cyst", "GSC_and_Spermatogonia", "Spermatids", "Spermatocytes"))

cell_type_abundances_summary <- data.frame(type = seurat_marker_ss$broad_type, 
                                           sample = seurat_marker_ss$sample, 
                                           treatment = seurat_marker_ss$treatment) %>% 
...
sum_data <- seurat_marker@meta.data %>% 
  group_by(celltype, treatment, sample) %>% 
  summarise(n = n()) %>% 
  spread(celltype,n)

sum_data$total <- rowSums(sum_data[,c(3:9)])
sum_data[,3:9] <- sum_data[,3:9]/sum_data$total

sum_data %>% 
  pivot_longer(c("Cyst", "Epithelial", "GSC_and_Spermatogonia", "Spermatocytes", "Spermatids"), names_to = "celltype", values_to = "prop") %>%
