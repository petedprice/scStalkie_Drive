...
PG_UMAP <- DimPlot(seurat_integrated,  label = TRUE, split.by = 'treatment')
ggsave("plots/UMAP_PopGroup.pdf", PG_UMAP, width = 16, height = 10)
seurat_integrated$celltype <- "Somatic"
seurat_integrated$celltype[seurat_integrated$integrated_snn_res.0.1 %in% c(3,7,4,0)] <- "Germ"
seurat_integrated$celltype[seurat_integrated$integrated_snn_res.0.1 %in% c(9,11,10)] <- "Unknown"
...
cell_type_abundances_summary <- data.frame(type = seurat_integrated$celltype, 
                                           sample = seurat_integrated$sample, 
                                           treatment = seurat_integrated$treatment) %>% 
...
sum_data <- seurat_integrated@meta.data %>% 
  group_by(celltype, treatment, sample) %>% 
  summarise(n = n()) %>% 
  spread(celltype,n)

sum_data$total <- rowSums(sum_data[,c(3:ncol(sum_data))])
sum_data[,3:ncol(sum_data)] <- sum_data[,3:ncol(sum_data)]/sum_data$total
