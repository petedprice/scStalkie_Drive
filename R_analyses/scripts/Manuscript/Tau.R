...
seurat_final@meta.data$celltype_broad[seurat_final$celltype %in% c("Primary spermatocytes", "GSC/Spermatogonia")] <- 
  "Pre-meiosis"
seurat_final@meta.data$celltype_broad[seurat_final$celltype %in% c("Late cyst", "Early cyst")] <- "Cyst"


sce <- seurat_final %>% subset(treatment == "ST") %>% as.SingleCellExperiment(assay = "RNA")

agg_cell_types <- aggregateAcrossCells(sce, id=colData(sce)[,c("celltype_broad", "sample")])
...
for (ct in unique(seurat_final$celltype_broad)){
  ...
  tau_keep_genes[[ct]] <- ST_keep_genes
}
#Calculating Tau

tau <- function(gene, cpm, y){
  expression <- cpm[gene,]
  expression <- log2(expression + 0.001)
...
tau_sum_func <- function(chrom, ct, taus, out = 'datas', tau_keep_genes = tau_keep_genes){
  tau_genes <- filter(taus, tau >= 0.85)%>% 
    filter(chr == chrom) %>% 
...
gois <- filter(tau_genes, chr == "Chr_X") %>% 
   filter(tau_type == "Post-meiosis") %>% 
   merge(ortholog_table, by.x = 'gene', by.y = 'REF_GENE_NAME') %>% 
   dplyr::select(gene) %>% unlist()
names(gois) <- NULL
length(gois)
#FeaturePlot(seurat_final, features = gois)
...
Idents(seurat_final) <- "celltype_broad"
markers <- seurat_final %>% 
  subset(., treatment == "ST")  %>%
  FindMarkers(., ident.1 = 'Post-meiosis', ident.2 = 'Pre-meiosis', 
              only.pos = T)

gois <- markers %>% 
  #filter(cluster == "Post-meiotic") %>% 
