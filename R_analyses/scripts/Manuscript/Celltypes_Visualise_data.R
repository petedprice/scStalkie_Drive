...
markers <- read.xlsx("data/MANUSCRIPT/DRIVE_MANUSCRIPT_SUPPLEMENTARY_TABLES.xlsx", sheet = 1)[,c(2,3,4,5,6)] %>% 
  filter(`DOI.(Source)` != "https://doi.org/10.1038/s41467-021-20897-y")
...
dps_all_figure <- DotPlot(seurat_integrated_ss, features = names(main_figure_markers), assay = "RNA")+coord_flip() + 
  scale_x_discrete(labels = main_figure_markers)
...
DGE <- FindAllMarkers(seurat_integrated, only.pos = F, min.pct.1 = 0.5, logfc.threshold = 1)

DGE <- DGE %>% 
  filter(p_val_adj < 0.05 & avg_log2FC > 1) %>% 
  merge(ortholog_table, by.x = 0, by.y = "REF_GENE_NAME")
DimPlot(seurat_integrated, group.by = 'integrated_snn_res.0.5', label = T)
